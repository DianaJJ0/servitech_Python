...
async function login(email, password) {
  // El backend Django espera username y password en el endpoint /usuarios/api/login/
  const res = await fetch("http://127.0.0.1:8080/usuarios/api/login/", {
    method: "POST",
  ...
  if (res.ok) {
    // Si el backend retorna un token, guárdalo
    if (data.token) {
